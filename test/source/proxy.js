...
describe('Use with Proxy', () => {
	...
	[ undefined, false, true ]
		.forEach((trackOnlyLastMutation) => {
			const scope = trackOnlyLastMutation ? 'last' : 'all';

			describe(`trackOnlyLastMutation = ${ JSON.stringify(trackOnlyLastMutation) }`, () => {
				const trap = new Trap(trackOnlyLastMutation);
				const proxy = new Proxy({ qux: true }, trap);
				let count = 0;

				it(`tracks ${ scope } mutation${ trackOnlyLastMutation ? '' : 's' }`, (next) => {
				...
				it(`tracks ${ scope } deletion${ trackOnlyLastMutation ? '' : 's' }`, (next) => {
					delete proxy.foo;
				...
				it(`tracks ${ scope } property definition${ trackOnlyLastMutation ? '' : 's' }`, (next) => {
					...
					Object.defineProperty(proxy, 'foo', { get: () => 'done', enumerable: false });
					count += Number(!trackOnlyLastMutation);
